...
public class WordRepository {
    ...
    private static MutableLiveData<List<Word>> mAllWords;

    WordRepository(Application application) {
        mWordDbHelper = WordDbHelper.getDatabase(application);
        mAllWords = new MutableLiveData<>();
        mAllWords.postValue(mWordDbHelper.getAllWords()); // setValue()
    }
    ...
    public void insert (Word word) {
        new insertAsyncTask(mWordDbHelper).execute(word);
    }
...
//    }
    ...
    private static class insertAsyncTask extends AsyncTask<Word, Void, Void> {
        ...
        private WordDbHelper mAsyncTaskWordDbHelper;

        insertAsyncTask(WordDbHelper dbHelper) {
            mAsyncTaskWordDbHelper = dbHelper;
        }
        ...
        protected Void doInBackground(final Word... words) {
            mAsyncTaskWordDbHelper.insert(words[0]);
            return null;
        }
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            mAllWords.postValue(mAsyncTaskWordDbHelper.getAllWords());
        }
    }
}
