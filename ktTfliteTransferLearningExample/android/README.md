...
### Code
* the `onViewCreated` method of `CameraFragment.kt` file provides the training button
* `setupModelPersonalization` method of the `TransferLearningHelper` class uses `FileUtil.loadMappedFile` method to load `model.tflite` flattbuffer file from the asset folder
* the `assets/model/model.tflite` is not used, since it is a empty model
